...
def make_app(settings: Settings):
    ...
    # Database
    database = Database(uri=settings.database_uri)
    ...
    if settings.debug:
        # If we are in Debug, we add a testing endpoint to create users
        ...
        @app.post('/debug/user/add')
        async def debug_user_add(user_info: UserAddInput):
            with database.session_factory() as session:
                user = User(id=user_info.id,
                            github_id=user_info.github_id, github_name=user_info.github_name,
                            reddit_id=user_info.reddit_id, reddit_name=user_info.reddit_name,
                            is_admin=user_info.is_admin, is_banned=False, username="")
                session.add(user)
                session.commit()
                return {"error": None}

        class UserRemoveInput(BaseModel):
            id: uuid.UUID

        @app.post('/debug/user/remove')
        async def debug_user_remove(user_info: UserRemoveInput):
            with database.session_factory() as session:
                user = session.query(User).where(User.id == user_info.id).one_or_none()
                if user is None:
